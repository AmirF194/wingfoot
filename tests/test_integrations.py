...
against it: unsigned must 403, botpass-signed must 200. This is the `403 -> 200`
...
from botpass.verifier import start_verifier
...
def test_signed_headers_verify_at_the_edge(verifier_and_identity):
    ...
    headers = botpass.signed_headers(f"{base}/whoami", identity=identity)
...
def test_requests_adapter(verifier_and_identity):
    ...
    resp = requests.get(f"{base}/whoami", auth=botpass.requests_auth(identity=identity))
...
def test_httpx_adapter(verifier_and_identity):
    ...
    resp = httpx.get(f"{base}/whoami", auth=botpass.httpx_auth(identity=identity))
    assert resp.status_code == 200
    assert resp.json()["verified"] is True
...
def test_missing_identity_is_a_clear_error(monkeypatch):
    ...
    monkeypatch.setattr("botpass.integrations.load_identity", lambda *a, **k: None)
    with pytest.raises(RuntimeError, match="botpass init"):
        botpass.signed_headers("https://example.com/")
